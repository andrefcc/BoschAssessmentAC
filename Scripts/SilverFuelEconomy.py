...
BRONZE_PATH = Path("C:/Users/e709117/Downloads/Assessment/Bronze/vehicles.csv.zip")
SILVER_FOLDER = Path("C:/Users/e709117/Downloads/Assessment/Silver")
SILVER_PARQUET = SILVER_FOLDER / "vehicles.parquet"
...
# 8.1) Standardize and uppercase on main columns that will be used for joins
UpperCaseColumns = ["MODEL", "MAKE", "DRIVE"]
for col in UpperCaseColumns:
    if col in df.columns:
        df[col] = df[col].astype("string").str.strip().str.upper() 

# 8.2) Rename join columns
df = df.rename(columns={"MAKE": "MANUFACTURER", "DRIVE": "DRIVETRAIN", "YEAR": "MODEL_YEAR"})
        
# 8.3) Standardize column "DRIVE_TRAIN" values
drive_map = {
...
df["DRIVETRAIN"] = df["DRIVETRAIN"].map(drive_map)

# 8.4) Convert ID column to string
